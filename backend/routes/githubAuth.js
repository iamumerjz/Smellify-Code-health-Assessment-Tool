...
const performAllAnalyses = async (extractedPath, projectId = null) => {
  console.log('\n' + '='.repeat(80));
  ...
  try {
    ...
    try {
      ...
      if (projectId) {
        console.log('Saving AI suggestions to database...');
        ...
        console.log('Suggestion counts:', {
          duplicates: aiSuggestions.duplicates?.length || 0,
          hooks: aiSuggestions.hooks?.length || 0,
          propDrilling: aiSuggestions.propDrilling?.length || 0,
          codeQuality: aiSuggestions.codeQuality?.length || 0
        });
      } else {
        console.log(' No projectId provided, skipping database save');
      }
...
router.post('/import-repository', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // 🚀 UPDATED: Run analysis with projectId for AI generation
    console.log('\n Starting combined code analysis (4 analyses + AI) for GitHub project in background...\n');
    
    const projectId = project._id;
    
    performAllAnalyses(processResult.downloadedPath, projectId) // 👈 PASS projectId
      .then(async (combinedResults) => {
        console.log('\n' + '='.repeat(80));
