...
import { Gem, Receipt, Eye, Settings, Plus, Upload, Github, X, FileText, FolderOpen, Star, GitFork, Clock, ExternalLink, RefreshCw, AlertCircle, Check, AlertTriangle, UserPlus, Menu, BarChart3, Package, Activity, LogOut, Award, CircleAlert, HelpCircle,  } from 'lucide-react';
import UserOnboarding from '../components/UserOnboarding';
import api from '../services/api';
import NotificationBell from '../components/NotificationBell';
import { useNotification } from '../components/NotificationPopup';
...
const getStatusIcon = (status) => {
  ...
  const stats = {
    ...
    analyzing: recentProjects.filter(p => p.analysisStatus === 'processing').length,
    completed: recentProjects.filter(p => p.analysisStatus === 'completed').length,
    issues: recentProjects.filter(p => p.analysisStatus === 'completed').length
  };
        ...
        <main className="flex-1 p-4 lg:p-8 overflow-auto">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 lg:gap-6 mb-6 lg:mb-8">
            <div className="bg-white rounded-xl p-4 lg:p-6 shadow-sm border border-gray-200">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <p className="text-xs lg:text-sm font-medium text-gray-600 mb-1">Scans Remaining</p>
                  <h3 className="text-2xl lg:text-3xl font-bold text-gray-900">{user?.remainingScans || 0}</h3>
                </div>
            ...
            <div className="bg-white rounded-xl p-4 lg:p-6 shadow-sm border border-gray-200">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <p className="text-xs lg:text-sm font-medium text-gray-600 mb-1">Total Projects</p>
                  <h3 className="text-2xl lg:text-3xl font-bold text-gray-900">{stats.totalProjects}</h3>
                </div>
            ...
            <div className="bg-white rounded-xl p-4 lg:p-6 shadow-sm border border-gray-200">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <p className="text-xs lg:text-sm font-medium text-gray-600 mb-1">Analyzing</p>
                  <h3 className="text-2xl lg:text-3xl font-bold text-gray-900">{stats.analyzing}</h3>
                </div>
            ...
            <div className="bg-white rounded-xl p-4 lg:p-6 shadow-sm border border-gray-200">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <h3 className="text-2xl lg:text-3xl font-bold text-gray-900">{stats.completed}</h3>
                </div>
                <div className="w-10 h-10 lg:w-12 lg:h-12 rounded-lg bg-green-50 flex items-center justify-center flex-shrink-0 ml-2">
            ...
            <div className="bg-white rounded-xl p-4 lg:p-6 shadow-sm border border-gray-200">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <p className="text-xs lg:text-sm font-medium text-gray-600 mb-1">Issues Found</p>
                  <h3 className="text-2xl lg:text-3xl font-bold text-gray-900">{stats.issues}</h3>
                </div>
          ...
          <div className="mb-6 lg:mb-8">
            {user?.remainingScans <= 0 ? (
                ...
                <div className="space-y-3 lg:space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm lg:text-base text-gray-600 mb-4">
                      You've used all your available scans. Purchase more to continue analyzing projects.
                    </p>
                  </div>
                  <button
                    onClick={() => window.location.href = '/plans'}
              ...
              >
                <div className="space-y-3 lg:space-y-4">
                  <div className="flex justify-center">
                    <div className={`w-12 h-12 lg:w-16 lg:h-16 rounded-full ${isDragOver ? 'bg-purple-100' : 'bg-gray-100'} flex items-center justify-center`}>
                      <FolderOpen className={`w-6 h-6 lg:w-8 lg:h-8 ${isDragOver ? 'text-[#5A33FF]' : 'text-gray-400'}`} />
                    </div>
                  </div>
                  <div>
                    <h3 className="text-base lg:text-lg font-semibold text-gray-900 mb-2">
                    ...
                    <p className="text-sm lg:text-base text-gray-500 mb-4">
                      Drag and drop a ZIP file containing your project, or click the button below
                    </p>
                      ...
                      {uploadStatus === 'uploading' && (
                        <div className="space-y-2">
                          <div className="flex items-center justify-center space-x-2">
                            ...
                            <span className="text-[#5A33FF] font-medium">Uploading and validating...</span>
                          </div>
                          ...
                          <p className="text-[#5A33FF] text-sm">{uploadProgress}% complete</p>
                        </div>
                      )}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200">
            <div className="px-4 lg:px-6 py-4 border-b border-gray-200 flex items-center justify-between">
              <h2 className="text-base lg:text-lg font-semibold text-gray-900">Recent Projects</h2>
              <button
                onClick={fetchRecentProjects}
                disabled={loadingProjects}
                className="p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg transition-colors disabled:opacity-50"
            ...
            <div className="p-4 lg:p-6">
              {loadingProjects ? (
                    ...
                    >
                      ...
                      <div className="flex-1 lg:ml-4">
                        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-1">
                          <h3 className="text-sm font-semibold text-gray-900 mb-2 lg:mb-0">{project.projectName}</h3>
                          <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold shadow-sm w-fit ${getStatusColor(project.analysisStatus)}`}>
                            {getStatusIcon(project.analysisStatus)}
                            <span className="ml-1.5 capitalize">{project.analysisStatus}</span>
                          </span>
                        </div>
                        {project.description && (
                ...
                <div className="text-center py-12">
                  <FileText className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                  <p className="text-gray-500 font-medium">No projects uploaded yet</p>
                  <p className="text-gray-400 text-sm mt-1">Upload your first MERN stack project to get started with code analysis</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl max-w-md w-full p-6 shadow-2xl">
            ...
            <div className="mb-6 p-4 bg-purple-50 rounded-lg border border-purple-100">
              <h4 className="text-sm font-semibold text-[#5A33FF] mb-2">Upload Requirements:</h4>
            ...
            <div className="space-y-3">
              ...
              <label className="cursor-pointer block p-5 border-2 border-dashed border-gray-300 rounded-lg hover:border-[#5A33FF] hover:bg-purple-50 transition-all">
                ...
                <div className="text-center">
                  <Upload className="w-8 h-8 text-gray-400 mx-auto mb-2" />
                  <p className="text-sm font-semibold text-gray-900">Manual Upload</p>
                  <p className="text-xs text-gray-500 mt-1">Select a ZIP file from your computer</p>
                </div>
              </label>
              ...
              {githubConnected && (
                ...
                >
                  <div className="text-center">
                    <Github className="w-8 h-8 text-gray-400 mx-auto mb-2" />
                    <p className="text-sm font-semibold text-gray-900">Import from GitHub</p>
                    <p className="text-xs text-gray-500 mt-1">Select a repository from your GitHub account</p>
                  </div>
                </button>
              )}
            </div>
                      ...
                      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between">
                        <div className="flex-1 mb-4 lg:mb-0">
                          <div className="flex flex-wrap items-center gap-2 mb-2">
                            <h4 className="text-base font-semibold text-gray-900">{repo.name}</h4>
                            {repo.isPrivate && (
            ...
            <div className="flex items-start space-x-3 mb-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-600 mb-4">
                  Repository: <span className="font-medium">{githubValidationError.repository.name}</span>
                </p>
                <div className="bg-red-50 rounded-lg p-4 mb-4">
                  <p className="text-sm text-red-800">{githubValidationError.error}</p>
                </div>
                {githubValidationError.requirements && (
                  <div className="bg-purple-50 rounded-lg p-4 border border-purple-100">
                    <h4 className="text-sm font-semibold text-[#5A33FF] mb-2">Requirements:</h4>
                    <ul className="text-xs text-gray-700 space-y-1">
